...
<tr class="newTableRow">
    ...
    <td>
        <select :value="index.type" @change="onIndexTypeChange"
            ...
            <option value=""></option>
            <option value="INDEX">{{ __('admin.database.index') }}</option>
            <option value="UNIQUE">{{ __('admin.database.unique') }}</option>
            <option value="PRIMARY">{{ __('admin.database.primary') }}</option>
        </select>
        <small v-if="column.composite" v-once>{{ __('admin.database.composite_warning') }}</small>
    </td>
...
@include('admin::tools.database.vue-components.database-types')
@include('admin::tools.database.vue-components.database-column-default')
    ...
    Vue.component('database-column', {
        ...
        methods: {
            ...
            onIndexTypeChange(event) {
                if (this.column.name == '') {
                    return toastr.error("{{ __('admin.database.name_warning') }}");
                }
