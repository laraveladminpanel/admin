...
<form role="form" action="{{ route('admin.database.crud.relationship') }}" method="POST">
			        ...
			        <div class="row">
			        	...
			        	@if(isset($dataType->id))
				            <div class="col-md-12 relationship_details">
				                ...
				                <span><input type="text" class="form-control" name="relationship_model" placeholder="{{ __('admin.database.relationship.namespace') }}" value="@if(isset($relationshipDetails->model)){{ $relationshipDetails->model }}@endif"></span>
				            </div>
				            ...
				            <div class="col-md-12 relationship_details_more">
				                <div class="well">
				                    <label>{{ __('admin.database.relationship.selection_details') }}</label>
				                    <p><strong>{{ __('admin.database.relationship.display_the') }} <span class="label_table_name"></span>: </strong>
				                        <select name="relationship_label" class="rowDrop select2" data-table="{{ $tables[0] }}" data-selected="">
				                        </select>
				                    </p>
				                    <p class="relationship_key"><strong>{{ __('admin.database.relationship.store_the') }} <span class="label_table_name"></span>: </strong>
