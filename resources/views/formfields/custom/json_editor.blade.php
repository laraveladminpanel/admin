<div class="form-group">
    <div id="{{ $row->field }}" data-theme="{{ @$options->theme }}" data-language="{{ @$options->language }}" class="ace_editor min_height_200"  data-name="{{ $row->display_name }}" name="{{ $row->field }}">@if(isset($dataTypeContent->{$row->field})){{ old($row->field, $dataTypeContent->{$row->field}) }}@elseif(isset($options->default)){{ old($row->field, $options->default) }}@else{{ old($row->field) }}@endif</div>
        ...
        $(function () {
            ...
            $('#crud-items').sortable({
                ...
                update: function (e, ui) {
                    var _rows = $('#crud-items').find('.row_order'),
                        _size = _rows.length;
