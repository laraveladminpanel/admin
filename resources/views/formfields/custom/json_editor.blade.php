<div class="form-group">
    <textarea id="json-input-{{ $row->field }}" class="resizable-editor" data-editor="json" data-name="{{ $row->display_name }}" name="{{ $row->field }}">@if(isset($dataTypeContent->{$row->field})){{ old($row->field, $dataTypeContent->{$row->field}) }}@elseif(isset($options->default)){{ old($row->field, $options->default) }}@else{{ old($row->field) }}@endif</textarea>
</div>
