...
@section('page_title', __('admin.generic.viewing').' '.__('admin.generic.settings'))

@section('css')
    <style>
        .panel-actions .admin-trash {
            cursor: pointer;
        }
        .panel-actions .admin-trash:hover {
            color: #e94542;
        }
        ...
        .admin .settings .nav-tabs .active a{
            border:0px;
        }
        ...
        .admin .settings .nav-tabs > li{
            margin-bottom:-1px !important;
        }

        .admin .settings .nav-tabs a{
            text-align: center;
            background: #f8f8f8;
            border: 1px solid #f1f1f1;
            position: relative;
        ...
        .admin .settings .nav-tabs > li > a:hover{
...
@section('page_header')
    <h1 class="page-title">
        <i class="admin-settings"></i> {{ __('admin.generic.settings') }}
    </h1>
@stop
...
@section('content')
    <div class="container-fluid">
        ...
        <div class="alert alert-info">
            ...
            <p>{{ __('admin.settings.usage_help') }} <code>setting('group.key')</code></p>
        </div>
        @endif
    </div>
    ...
    <div class="page-content settings container-fluid">
        <form action="{{ route('admin.settings.update') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div id="{{ str_slug($group) }}" class="tab-pane fade in @if($loop->first) active @endif">
                            ...
                            <div class="panel-heading">
                                <h3 class="panel-title">
                                    {{ $setting->display_name }} @if(config('admin.show_dev_tips'))<code>setting('{{ $setting->key }}')</code>@endif
                                </h3>
                                <div class="panel-actions">
                                    <a href="{{ route('admin.settings.move_up', $setting->id) }}">
                                        <i class="sort-icons admin-sort-asc"></i>
                                    </a>
                                    <a href="{{ route('admin.settings.move_down', $setting->id) }}">
                                        <i class="sort-icons admin-sort-desc"></i>
                                    </a>
                                    <i class="admin-trash"
                                       data-id="{{ $setting->id }}"
                                    ...
                                    @elseif($setting->type == "image" || $setting->type == "file")
                                        @if(isset( $setting->value ) && !empty( $setting->value ) && Storage::disk(config('admin.storage.disk'))->exists($setting->value))
                                            <div class="img_settings_container">
                                                <a href="{{ route('admin.settings.delete_value', $setting->id) }}" class="admin-x"></a>
            ...
            <button type="submit" class="btn btn-primary pull-right">{{ __('admin.settings.save') }}</button>
        </form>
        ...
        <div class="panel" style="margin-top:10px;">
            <div class="panel-heading new-setting">
                <hr>
                <h3 class="panel-title"><i class="admin-plus"></i> {{ __('admin.settings.new') }}</h3>
            </div>
            <div class="panel-body">
                <form action="{{ route('admin.settings.store') }}" method="POST">
                    {{ csrf_field() }}
                    <div class="col-md-3">
                        <label for="display_name">{{ __('admin.generic.name') }}</label>
                        <input type="text" class="form-control" name="display_name" placeholder="{{ __('admin.settings.help_name') }}" required="required">
                    </div>
                    <div class="col-md-3">
                        <label for="key">{{ __('admin.generic.key') }}</label>
                        <input type="text" class="form-control" name="key" placeholder="{{ __('admin.settings.help_key') }}" required="required">
                    </div>
                    <div class="col-md-3">
                        <label for="asdf">{{ __('admin.generic.type') }}</label>
                        <select name="type" class="form-control" required="required">
                            <option value="">{{ __('admin.generic.choose_type') }}</option>
                            <option value="text">{{ __('admin.form.type_textbox') }}</option>
                            <option value="text_area">{{ __('admin.form.type_textarea') }}</option>
                            <option value="rich_text_box">{{ __('admin.form.type_richtextbox') }}</option>
                            <option value="code_editor">{{ __('admin.form.type_codeeditor') }}</option>
                            <option value="checkbox">{{ __('admin.form.type_checkbox') }}</option>
                            <option value="radio_btn">{{ __('admin.form.type_radiobutton') }}</option>
                            <option value="select_dropdown">{{ __('admin.form.type_selectdropdown') }}</option>
                            <option value="file">{{ __('admin.form.type_file') }}</option>
                            <option value="image">{{ __('admin.form.type_image') }}</option>
                        </select>
                    </div>
                    <div class="col-md-3">
                        <label for="group">{{ __('admin.settings.group') }}</label>
                    ...
                    <div class="col-md-12">
                        <a id="toggle_options"><i class="admin-double-down"></i> {{ mb_strtoupper(__('admin.generic.options')) }}</a>
                        <div class="new-settings-options">
                            <label for="options">{{ __('admin.generic.options') }}
                                <small>{{ __('admin.settings.help_option') }}</small>
                            </label>
                            ...
                            <div id="valid_options" class="alert-success alert" style="display:none">{{ __('admin.json.valid') }}</div>
                            <div id="invalid_options" class="alert-danger alert" style="display:none">{{ __('admin.json.invalid') }}</div>
                        </div>
                    </div>
                    <div style="clear:both"></div>
                    <button type="submit" class="btn btn-primary pull-right new-setting-btn">
                        <i class="admin-plus"></i> {{ __('admin.settings.add_new') }}
                    </button>
                    <div style="clear:both"></div>
                </form>
            </div>
            ...
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">
                        <i class="admin-trash"></i> {!! __('admin.settings.delete_question', ['setting' => '<span id="delete_setting_title"></span>']) !!}
                    </h4>
                </div>
                <div class="modal-footer">
                    <form action="{{ route('admin.settings.delete', ['id' => '__id']) }}" id="delete_form" method="POST">
                        {{ method_field("DELETE") }}
                        {{ csrf_field() }}
                        <input type="submit" class="btn btn-danger pull-right delete-confirm" value="{{ __('admin.settings.delete_confirm') }}">
                    </form>
...
@section('javascript')
    <script>
        $('document').ready(function () {
            $('#toggle_options').click(function () {
                ...
                if ($('#toggle_options .admin-double-down').length) {
                    $('#toggle_options .admin-double-down').removeClass('admin-double-down').addClass('admin-double-up');
                } else {
                    $('#toggle_options .admin-double-up').removeClass('admin-double-up').addClass('admin-double-down');
                }
            });

            $('.admin-trash').click(function () {
    ...
    $(".group_select_new").select2({
        tags: true,
        width: 'resolve',
        placeholder: '{{ __("admin.generic.select_group") }}'
    });
